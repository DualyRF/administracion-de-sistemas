...
function Gestionar-Usuarios {
    ...
    Write-Host "  5) Volver"                           
    Write-Host ""
    $opcion = Read-Host "Seleccione una opcion" -ForegroundColor Cyan
...
function resetearServidor {
    ...
    if (Get-WebSite -Name $FTP_SITE_NAME  -ErrorAction SilentlyContinue) {
        & "$env:SystemRoot\System32\inetsrv\appcmd.exe" stop site $FTP_SITE_NAME  2>$null
        Remove-WebSite -Name $FTP_SITE_NAME 
        Write-Log "Sitio IIS '$FTP_SITE_NAME ' eliminado." "OK"
    }
...
function menuPrincipal {
    do {
        ...
        Write-Host "  3. Ver estado y diagnostico                   "
        Write-Host "  4. Reiniciar servidor FTP                     "
        Write-Host "  5. Verificar instalacion de IIS y FTP         "
        Write-Host "  6. Reset completo del servidor (borrar todo)  "
        Write-Host "  7. Listar usuarios activos                    "
        Write-Host "  8. Salir                                      "
        Write-Host "------------------------------------------------" -ForegroundColor Magenta
        Write-Host ""
        ...
        switch ($opcion) {
            ...
            "2" {
                if (-not (Verificar-Instalacion)) {
                    Print-Error "IIS/FTP no instalado. Ejecuta la opcion 1 primero."
                    Read-Host "`nPresiona Enter para continuar"
                } else {
                    Gestionar-Usuarios
                }
